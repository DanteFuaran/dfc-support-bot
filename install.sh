...
INSTALL_DIR="/dfc-online/dfc-support-bot"
SERVICE_FILE="/etc/systemd/system/dfc-support-bot.service"
CLI_FILE="/usr/local/bin/dfc-sb"
LOCK_FILE="/tmp/dfc-support-bot-install.lock"
...
cleanup_on_fail() {
  ...
  systemctl stop dfc-support-bot.service 2>/dev/null || true
  systemctl disable dfc-support-bot.service 2>/dev/null || true
  rm -rf "$INSTALL_DIR"
...
KEYS_URL="https://raw.githubusercontent.com/DanteFuaran/dfc-support-bot/main/license"
...
if [ -d "$INSTALL_DIR" ] && [ "$(ls -A "$INSTALL_DIR" 2>/dev/null || true)" ]; then
  ...
  case "$confirm" in
    [yY][eE][sS]|[yY])
      ...
      mkdir -p "$INSTALL_DIR"
      cd "$INSTALL_DIR"
      git clone https://github.com/DanteFuaran/dfc-support-bot.git . >/dev/null 2>&1 &
...
Environment="VIRTUAL_ENV=/dfc-online/dfc-support-bot/.venv"
Environment="PATH=/dfc-online/dfc-support-bot/.venv/bin:/usr/bin:/bin"
...
cat > "$CLI_FILE" << 'EOF'
#!/bin/bash
SERVICE="dfc-support-bot.service"
INSTALL_DIR="/dfc-online/dfc-support-bot"
...
reinstall_bot() {
  ...
  case "$confirm" in
    [yY][eE][sS]|[yY])
      ...
      bash <(curl -s "https://raw.githubusercontent.com/DanteFuaran/dfc-support-bot/main/install.sh")
      exit 0
      ;;
    *)
...
sleep 1
systemctl daemon-reload >/dev/null 2>&1
systemctl enable dfc-support-bot.service >/dev/null 2>&1
systemctl restart dfc-support-bot.service >/dev/null 2>&1
sleep 1

if systemctl is-active --quiet dfc-support-bot.service; then
  echo -e "${GREEN}✅${WHITE} Бот успешно запущен!${NC}"
else
  echo -e "${RED}❌ Не удалось запустить бота автоматически.${NC}"
  echo -e "${YELLOW}Попробуйте вручную: systemctl start dfc-support-bot.service${NC}"
fi
...
echo -e "${BLUE}==========================================${NC}"
echo -e "${BLUE}Меню управления ботом:${NC} ${YELLOW}dfc-sb${NC}\n"
